export default function Cell(props) {
  ...
  return (
    <div draggable={false} onMouseEnter={() => onMouseEnter(row, col)} onMouseLeave={() => onMouseLeave(row, col)} onMouseDown={() => onMouseDown(row, col)} onMouseUp={() => onMouseUp(row, col)} id={`cell-${props.row}-${props.col}`} className={`sp-cell ${tag}`}>
